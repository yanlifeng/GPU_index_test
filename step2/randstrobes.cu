...
//__device__ Randstrobe RandstrobeIterator::gpu_get(unsigned int strobe1_index) const {
//    unsigned int w_end = std::min(static_cast<size_t>(strobe1_index + w_max), (*gpu_syncmers).size() - 1);
//
...
//        assert(i < (*gpu_syncmers).size());
//
//        // Method 3' skew sample more for prob exact matching
//        std::bitset<64> b = (strobe1.hash ^ (*gpu_syncmers)[i].hash) & q;
...
//    std::vector<Syncmer> syncmers = canonical_syncmers(seq, parameters.syncmer);
//    if (syncmers.empty()) {
//        return randstrobes;
//    }
//
